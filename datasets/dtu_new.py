...
class MVSDataset(Dataset):
    ...
    def rgb2yCbCr(self,input_im):
        im_flat = input_im.reshape(-1, 3)
        mat = np.array([[0.257, -0.148, 0.439],[0.564, -0.291, -0.368],[0.098, 0.439, -0.071]])
        bias = np.array([16.0 / 255.0, 128.0 / 255.0, 128.0 / 255.0])
        temp = np.matmul(im_flat,mat) + bias
        out = temp.reshape(3, input_im.shape[1], input_im.shape[2])
        return out
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            mask_filename_hr = os.path.join(self.datapath, 'Depths_raw/Depths/{}/depth_visual_{:0>4}.png'.format(scan, vid))
            depth_filename_hr = os.path.join(self.datapath, 'Depths_raw/Depths/{}/depth_map_{:0>4}.pfm'.format(scan, vid))
            ...
            depths.append(depth_ms)
            imgs.append(img)

        #all
        # for n,i in enumerate(imgs):
        #     imgs[n]=self.rgb2yCbCr(i.transpose(2,0,1))[1:].transpose(1,2,0)
        # masks = np.stack(masks)
        # depths = np.stack(depths)
        refimgs=imgs[1:].copy()
        for n,i in enumerate(refimgs):
            refimgs[n]=i.transpose([2,0,1])

        imgs = np.stack(imgs).transpose([0, 3, 1, 2])

        #ms proj_mats   #因为不同stage特征分辨率不同，所以映射矩阵也不同（乘以不同的倍数）
