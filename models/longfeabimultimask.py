...
class Conv3d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 relu=True, bn=True, bn_momentum=0.1, init_method="xavier",padding=1, **kwargs):
        super(Conv3d, self).__init__()
        self.out_channels = out_channels
        self.kernel_size = kernel_size
        assert stride in [1, 2]
        self.stride = stride
        self.convs1 = nn.Conv3d(in_channels, in_channels//4, kernel_size=(1, 1, 1), padding=(0, 0, 0),stride=stride,
                              bias=(not bn), **kwargs)
        self.convs3 = nn.Conv3d(in_channels, in_channels//4, kernel_size=(1, 3, 3), padding=(0, 1, 1),stride=stride,
                              bias=(not bn), **kwargs)
        self.convs5 = nn.Conv3d(in_channels, in_channels//4, kernel_size=(1, 5, 5), padding=(0, 2, 2),stride=stride,
                              bias=(not bn), **kwargs)
        self.convs7 = nn.Conv3d(in_channels, in_channels//4, kernel_size=(1, 7, 7), padding=(0, 3, 3),stride=stride,
                              bias=(not bn), **kwargs)
        self.conv2 = nn.Conv3d(in_channels, out_channels, kernel_size=(7, 1, 1), padding=(3, 0, 0),stride=1,
                              bias=(not bn), **kwargs)
    ...
    def init_weights(self, init_method):
        """default initialization"""
        init_uniform(self.convs1, init_method)
        init_uniform(self.convs3, init_method)
        init_uniform(self.convs5, init_method)
        init_uniform(self.convs7, init_method)
        init_uniform(self.conv2, init_method)
        if self.bn is not None:
            init_bn(self.bn)
...
class Deconv3d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 relu=True, bn=True, bn_momentum=0.1, init_method="xavier", padding=1, output_padding=1):
        super(Deconv3d, self).__init__()
        self.out_channels = out_channels
        assert stride in [1, 2]
        self.stride = stride

        self.convs1 = nn.ConvTranspose3d(in_channels, in_channels//4, kernel_size=(1, 1, 1), padding=(0, 0, 0),stride=stride,output_padding=1,
                              bias=(not bn))
        self.convs3 = nn.ConvTranspose3d(in_channels, in_channels//4, kernel_size=(1, 3, 3), padding=(0, 1, 1),stride=stride,output_padding=1,
                              bias=(not bn))
        self.convs5 = nn.ConvTranspose3d(in_channels, in_channels//4, kernel_size=(1, 5, 5), padding=(0, 2, 2),stride=stride,output_padding=1,
                              bias=(not bn))
        self.convs7 = nn.ConvTranspose3d(in_channels, in_channels//4, kernel_size=(1, 7, 7), padding=(0, 3, 3),stride=stride,output_padding=1,
                              bias=(not bn))
        self.conv2 = nn.ConvTranspose3d(in_channels, out_channels, kernel_size=(7, 1, 1), padding=(3, 0, 0),stride=1,output_padding=0,
                              bias=(not bn))
        

        # self.conv = nn.ConvTranspose3d(in_channels, out_channels, kernel_size, stride=stride,
        #                                bias=(not bn), padding=1, output_padding=1)
        self.bn = nn.BatchNorm3d(out_channels, momentum=bn_momentum) if bn else None
        self.relu = relu
...
class volumeweight(nn.Module):
    def __init__(self):
        super(volumeweight, self).__init__()
        self.conv1_1 = ConvBnReLU(64, 32)
        self.conv2_1 = ConvBnReLU(32, 32)
        self.conv3_1 = ConvBnReLU(32, 32)
        self.conv1_2 = ConvBnReLU(32, 16)
        self.conv2_2 = ConvBnReLU(16, 16)
        self.conv3_2 = ConvBnReLU(16, 16)
        self.conv1_3 = ConvBnReLU(16, 8)
        self.conv2_3 = ConvBnReLU(8, 8)
        self.conv3_3 = ConvBnReLU(8, 8)

    def forward(self, ref, source):
        concat = torch.cat([ref, source], dim=1)
        if concat.shape[1] == 64:
            weight = self.conv3_1(self.conv2_1(self.conv1_1(concat)))
        elif concat.shape[1] == 32:
            weight = self.conv3_2(self.conv2_2(self.conv1_2(concat)))
        elif concat.shape[1] == 16:
            weight = self.conv3_3(self.conv2_3(self.conv1_3(concat)))
...
def get_cur_depth_range_samples(cur_depth, ndepth, depth_inteval_pixel, shape, max_depth=192.0, min_depth=0.0):
    ...
    #stage 2的深度范围是上一stage深度值附近的值。。基于上一stage输出
...
def get_depth_range_samples(cur_depth, ndepth, depth_inteval_pixel, device, dtype, shape,
    ...
    if cur_depth.dim() == 2: #stage 1深度范围是给定的
        cur_depth_min = cur_depth[:, 0]  # (B,)
        cur_depth_max = cur_depth[:, -1]
        new_interval = (cur_depth_max - cur_depth_min) / (ndepth - 1)  # (B, )
        #0到ndepth（采样个数）均匀采样，乘以采样间隔。每一项再加上最小深度值
    ...
    else: #stage2，3....的深度值范围是在上一stage输出的深度值的基础上左右摇摆n/2*interval
