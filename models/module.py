...
class CostRegNet(nn.Module):
    def __init__(self, in_channels, base_channels):
        ...
        if isinstance(in_channels,list):
            self.convinput1_1 = Conv3d(in_channels[0], base_channels, padding=1)
            self.convinput2_1 = Conv3d(base_channels, base_channels, padding=1)
            self.convca_1 = Conv3d(base_channels, base_channels, kernel_size=1,padding=0)

            self.convinput1_2 = Conv3d(in_channels[1], base_channels, padding=1)
            self.convinput2_2 = Conv3d(base_channels, base_channels, padding=1)
            self.convca_2 = Conv3d(base_channels, base_channels, kernel_size=1,padding=0)

            self.convinput1_3 = Conv3d(in_channels[2], base_channels, padding=1)
            self.convinput2_3 = Conv3d(base_channels, base_channels, padding=1)
            self.convca_3 = Conv3d(base_channels, base_channels, kernel_size=1,padding=0)
    ...
    def forward(self, x,stage=10):

        if stage!=10:
            if stage ==0:
                x1 = self.convinput1_1(x)
                x2 = self.convinput2_1(x1)
                ca = self.convca_1(x2)
                da = torch.softmax(ca.mean(4).mean(3),2)
                da = da.unsqueeze(3).unsqueeze(4).repeat(1,1,1,x2.shape[3],x2.shape[4])
                inputs = da*x2
            elif stage ==1:
                x1 = self.convinput1_2(x)
                x2 = self.convinput2_2(x1)
                ca = self.convca_2(x2)
                da = torch.softmax(ca.mean(4).mean(3),2)
                da = da.unsqueeze(3).unsqueeze(4).repeat(1,1,1,x2.shape[3],x2.shape[4])
                inputs = da*x2
            else:
                x1 = self.convinput1_3(x)
                x2 = self.convinput2_3(x1)
                ca = self.convca_3(x2)
                da = torch.softmax(ca.mean(4).mean(3),2)
                da = da.unsqueeze(3).unsqueeze(4).repeat(1,1,1,x2.shape[3],x2.shape[4])
                inputs = da*x2
