...
class DepthNet(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, num_depth, cost_regularization, prob_volume_init=None):
        ...
        for src_fea, src_proj in zip(src_features, src_projs):#将不同src的feature和投影矩阵配对
            ...
            warped_volume = homo_warping(src_fea, src_proj_new, ref_proj_new, depth_values) #[B, C, Ndepth, H, W]
            # warped_volume = homo_warping(src_fea, src_proj[:, 2], ref_proj[:, 2], depth_values)

            cost_volume = torch.cat([volume_sum,warped_volume],1)
            cost_reg = cost_regularization(cost_volume)
            depth_volume = cost_reg[:,0]#b,n,h,w
            mask_volume = cost_reg[:,1]
            depth_prob_volume = F.softmax(depth_volume, dim=1)
            mask_prob_volume = F.softmax(mask_volume, dim=1)
            depth = depth_regression(depth_prob_volume, depth_values=depth_values)#b,h,w
            mask = depth_regression(mask_prob_volume, depth_values=depth_values)#b,h,w
            depth_list.append(depth.unsqueeze(1))
...
class CascadeMVSNet(nn.Module):
    def __init__(self, refine=False, ndepths=[48, 32, 8], depth_interals_ratio=[4, 2, 1], share_cr=False,
        ...
        if self.share_cr:
            ...
            self.cost_regularization = nn.ModuleList([CostRegNet(in_channels=2*self.feature.out_channels[i],
