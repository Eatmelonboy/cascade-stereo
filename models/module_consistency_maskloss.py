...
def cas_mvsnet_loss(inputs, depth_gt_ms, mask_ms,warpdepths,warpmask,predmask, **kwargs):
    #predmask [b,numview,depthnum,h,w]
    #warpmask [b,1,h,w]
    depth_loss_weights = kwargs.get("dlossw", None)
    # print(warpmask[0].shape)
    # print(predmask.shape)
    # input('t')
    total_loss = torch.tensor(0.0, dtype=torch.float32, device=mask_ms["stage1"].device, requires_grad=False)
    consistencyloss= torch.tensor(0.0, dtype=torch.float32, device=mask_ms["stage1"].device, requires_grad=False)
    ...
    for num ,i in enumerate(warpmask):
        # print(predmask[:,num,0].shape)
        # input('t')
        maskloss=F.l1_loss(i.squeeze(1), predmask[:,num,0], reduction='mean')
    for tem in warpdepths:
        consistencyloss+=F.smooth_l1_loss(inputs['depth'][mask], tem[mask], reduction='mean')

    total_loss += consistencyloss*0.5
    total_loss += maskloss
