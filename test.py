...
parser.add_argument('--testpath', default='/media/public/disk0/zzy/mvsnet/DTUtest/dtu',help='testing data dir for some scenes')
...
#filter by gimupa
parser.add_argument('--fusibile_exe_path', type=str, default='../cas_xconv/fusibile-master/fusibile')
...
def filter_depth(pair_folder, scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        ref_depth_est = read_pfm(os.path.join(out_folder, 'scan77/depth_map_{:0>4}.pfm'.format(ref_view)))[0]
        ref_depth_est = cv2.resize(ref_depth_est,(1152,864))
        ...
        for src_view in src_views:
            print('t3_{}'.format(src_view))
            # camera parameters of the source view
            src_intrinsics, src_extrinsics = read_camera_parameters(
                os.path.join(scan_folder, 'cams/{:0>8}_cam.txt'.format(src_view)))
            # the estimated depth of the source view
            src_depth_est = read_pfm(os.path.join(out_folder, 'scan77/depth_map_{:0>4}.pfm'.format(src_view)))[0]

            src_depth_est = cv2.resize(src_depth_est,(1152,864))
            print('t3_1_{}'.format(src_view))
        ...
        # depth_est_averaged = (sum(all_srcview_depth_ests) ) / (geo_mask_sum + 1)
        ref_depth_est[~(geo_mask_sum>0)]=0
        print('t3')
        save_pfm(os.path.join(out_folder, 'scan77/rep_depth_map_{:0>4}.pfm'.format(ref_view)), ref_depth_est.astype(np.float32))
